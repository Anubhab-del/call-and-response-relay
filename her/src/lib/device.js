...
function prefersSaveData() {
  return typeof navigator > `u` ? !1 : !!navigator.connection?.saveData;
}
...
function isSmallPhone() {
  return typeof window > `u` ? !1 : Math.min(window.innerWidth, window.innerHeight) <= 380;
}
...
function cappedPixelRatio(e) {
  return typeof window > `u` ? 1 : Math.max(1, Math.min(e, window.devicePixelRatio || 1));
}

function supportsFullscreen() {
  return typeof document > `u` || /iP(hone|ad|od)/.test(navigator.userAgent)
    ? !1
...
function startTilt() {
  if (typeof window > `u` || isStill()) return () => {};
  let e = document.documentElement,
    t = 0,
    n = 0,
    r = 0,
